...
const Top_First = ({ id, go }) => {
		...
		async function makeUsersList() {
			await bridge.send("VKWebAppCallAPIMethod", {"method": "users.get", "request_id": "34test", "params": {"user_ids":
				...
				+ "603535757, "										// Twi Arwe
				+ "564115201, diman19911, allaworld", "fields": "photo_200, city", "v":"5.131", "access_token":"cd4e738acd4e738acd4e738a93cd37cec6ccd4ecd4e738aac231fbb41d26d522accbf95"}}).then(data => {
					let i = 1;
	...
	return (
		<Panel id={id}>
			...
			<Group header={<Header mode="secondary">Первые последователи</Header>}>
				...
				<Cell
					before={topList_First["6"].photo_200 ? <Avatar src={topList_First["6"].photo_200}/> : null}
					description={topList_First["6"].city && topList_First["6"].city.title ? "г. " + topList_First["6"].city.title : ''}
				>
					{`${topList_First["6"].first_name} ${topList_First["6"].last_name}`}
				</Cell>
			</Group>}
